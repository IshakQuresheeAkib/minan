  ...
  return (
    ...
    >
      <header className="sticky top-0 z-40 flex items-center justify-between bg-background/95 px-4 pb-3 pt-3 backdrop-blur-md lg:hidden">
        <div className="size-11 animate-pulse rounded-full bg-background" />
        <div className="size-11 animate-pulse rounded-full bg-background" />
      </header>

      <div className="mx-auto w-full max-w-6xl flex-1 px-4 pb-36 lg:px-8 lg:pb-16 lg:pt-6">
        <div className="mb-4 hidden h-4 w-56 animate-pulse rounded bg-background lg:block" />
              ...
              <div className="lg:flex lg:gap-4">
                <div className="mb-5 hidden flex-col gap-3 lg:mb-0 lg:flex">
                  {Array.from({ length: 4 }, (_, index) => (
                    <div
                      key={index}
                      className="size-16 animate-pulse rounded-xl bg-background"
                    />
                  ))}
                </div>
                <div className="h-[380px] w-full animate-pulse rounded-[32px] bg-background lg:h-[520px] lg:flex-1 lg:rounded-2xl" />
              </div>
              <div className="mt-5 flex justify-center gap-2 lg:hidden">
                {Array.from({ length: 3 }, (_, index) => (
                  <div
                    key={index}
                    className="size-2.5 animate-pulse rounded-full bg-background"
                  />
                ))}
              </div>
            </section>
          ...
          <div>
            <div className="mb-4 hidden items-start justify-between gap-4 lg:flex">
              <div className="space-y-3">
                <div className="h-9 w-72 animate-pulse rounded bg-background" />
                <div className="h-8 w-36 animate-pulse rounded bg-background" />
              </div>
              <div className="size-11 animate-pulse rounded-full bg-background" />
            </div>

            <div className="mb-2 h-7 w-64 animate-pulse rounded bg-background lg:hidden" />
            <div className="mb-6 h-6 w-28 animate-pulse rounded bg-background lg:hidden" />
            <div className="mb-6 ml-auto h-9 w-28 animate-pulse rounded-full bg-background lg:ml-0" />
            ...
            <div className="mt-8 hidden items-center gap-3 lg:flex">
              ...
              <div className="h-12 w-32 animate-pulse rounded-full bg-background" />
            </div>

            <div className="mt-6 hidden h-12 w-full animate-pulse rounded-full bg-background lg:block" />
      ...
      <footer className="fixed inset-x-0 bottom-0 z-50 flex flex-col gap-3 bg-background/80 px-4 pb-8 pt-4 backdrop-blur-md lg:hidden">
        <div className="h-11 w-full animate-pulse rounded-full bg-background" />
        <div className="flex items-center gap-4">
          <div className="h-14 w-32 animate-pulse rounded-full bg-background" />
          <div className="h-14 flex-1 animate-pulse rounded-full bg-background" />
        </div>
      </footer>
    </div>
  );
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {Array.from({ length: rows }, (_, index) => (
          <div
            key={index}
            className="h-10 w-20 animate-pulse rounded-full bg-background"
          />
        ))}
      </div>
    </div>
