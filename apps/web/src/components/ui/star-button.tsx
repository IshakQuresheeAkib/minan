...
type StarButtonStyle = CSSProperties & {
  "--duration": number;
  "--light-width": string;
  "--light-color": string;
  "--border-width": string;
...
type StarButtonVisualProps = {
  backgroundColor?: string;
};

export function StarButtonVisual({
  backgroundColor = "currentColor",
}: StarButtonVisualProps) {
  return (
    <>
      <div
        data-slot="star-button-light"
      ...
      />
      <div
        className="pointer-events-none absolute inset-0 z-[4] overflow-hidden rounded-[inherit] border-black/10 text-white dark:border-white/15 dark:text-black"
...
export function StarButton({
  ...
  return (
    <button
      ...
      className={cn(
        "group/star-button relative z-[3] inline-flex h-10 items-center justify-center gap-2 overflow-hidden rounded-3xl px-4 py-2 text-sm font-medium whitespace-nowrap disabled:pointer-events-none disabled:opacity-50",
        className,
      )}
    >
      <StarButtonVisual backgroundColor={backgroundColor} />
