...
export async function processNextNotification(
  ...
  try {
    const delivery = await email.send({ to: notification.recipient_email, ...emailContent(notification) });
    await NotificationOutbox.updateOne(
      { _id: notification._id, status: "processing" },
      { $set: { status: "sent", sent_at: now, provider_message_id: delivery.id }, $unset: { locked_at: 1, last_error: 1 } },
    );
